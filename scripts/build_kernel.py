...
if __name__ == "__main__":
    ...
    ap.add_argument("--route", choices=["es", "ppo", "p1", "p1axes"], default="es")
    ...
    a = ap.parse_args()
    if a.route in ("p1", "p1axes"):
        # default 6000 updates/cell ≈ 1e8 env steps (override with --updates)
        u = a.updates if a.updates != 8000 else 6000
        (build_p1 if a.route == "p1" else build_p1axes)(a.session, u, a.tag)
    else:
